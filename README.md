...
├── .env                     # API key (not committed / not shared)
...
| `list\_files(directory, extension=None)` | Lists files in a directory, optionally filtered by extension. Returns file name, path, size, and last modified date. |
...
You can get an API key from \[console.anthropic.com](https://console.anthropic.com/). Note that using the API requires adding billing credits to your Anthropic account (a few dollars is more than enough for this project).
...
The core function is `run\_conversation(user\_message)` in `llm\_file\_assistant.py`. It takes a natural language query, lets Claude decide which tools to call (looping until Claude has enough information), and returns a final answer.
...
Each query may trigger multiple tool calls in sequence — for example, "Find resumes mentioning Python" first calls `list\_files` to discover what's in the folder, then calls `search\_in\_file` on each file found. The terminal output shows each tool call as it happens, e.g.:
...
3\. The Python code looks up the matching function in `available\_functions`, runs it with the arguments Claude provided, and packages the result.
...
The `resumes/` folder contains 7 dummy resumes across three formats, covering a mix of skill sets (Python, Java, JavaScript, DevOps, data analysis) so that search and read behavior can be tested meaningfully:
